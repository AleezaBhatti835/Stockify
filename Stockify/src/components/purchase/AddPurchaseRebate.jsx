    ...
    <div className="dashboard-wrapper" style={{ width: '100%' }}>
      ...
      {purchase && (
        <div className="card" style={{ display: 'flex', gap: 'var(--space-xl)', backgroundColor: 'var(--primary-light)', border: '1px dashed var(--btn-border)' }}>
          ...
          <span style={{ fontSize: '14px', color: 'var(--text-main)' }}><strong>Supplier:</strong> {purchase.supplier?.contactPerson || purchase.supplier?.companyName || 'Unknown'}</span>
        </div>
      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                ...
                <th style={tableStyles.th}>Purchased Qty</th>
            ...
            <tbody>
              {lineItems.length === 0 ? (
                <tr><td colSpan="7" style={tableStyles.emptyCell}>No invoice selected or no items available.</td></tr>
              ) : (
                lineItems.map((row, index) => (
                  <tr key={row.productId} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    <td style={tableStyles.td}>{row.productName}</td>
                    <td style={tableStyles.td}>{row.purchasedQuantity}</td>
